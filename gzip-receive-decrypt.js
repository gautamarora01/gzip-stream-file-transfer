import { createServer } from 'http';
import { createWriteStream } from 'fs';
import { createGunzip } from 'zlib';
import { basename, join } from 'path';
import { createDecipheriv, randomBytes } from 'crypto';

const secret = randomBytes(24);
console.log(`Generated Secret: ${secret.toString('hex')}`);

//Flow of data 
// CLient Reads from the filesystem -> Client Compresses the data -> Client sends to server
// Server Receives the data -> Server decompresses data -> Server writes the data to the disk

//We are currently on the receiving side
//here req is a stream object, which allows 
//server to receive request data in chunks from the network

const server = createServer((req,res)=>{

    const filename = basename(req.headers['x-filename']);
    const iv = Buffer.from(req.headers['x-initialization-vector'],'hex');
    const destFilename = join('received_files', filename);

    console.log(`File request received: ${filename}`);
    
    req
    .pipe(createDecipheriv('aes192', secret, iv))
    .pipe(createGunzip())
    .pipe(createWriteStream(destFilename))
    .on('finish', () => {
        res.writeHead(201,{
            'content-type': 'text/plain'
        })
        res.end('OK\n')
        console.log(`File Saved: ${destFilename}`);
    });
});


server.listen(3000, ()=>{
    console.log('Listening on Port', 3000);
})